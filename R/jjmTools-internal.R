...
.diagnostics <- function(jjm.out,jjm.in,jjm.ypr){
  ...
  inputPlots$weightByCohortFleet <- pic
  ...
  inputPlots$weightByCohortSurvey <- pic
  ...
  if(.an(ageFleets)[1] != 0){
    ...
    inputPlots$ageFleetsPlots <- ageFleetsPlots
  }
  ...
  if(length(which(jjm.in$Inumageyears>0))>0){
    ...
    inputPlots$ageCompositionSurvey1 <- pic
    ...
    inputPlots$ageCompositionSurvey2 <- pic
  }
  ...
  inputPlots$weightPopulation <- pic
  ...
  inputPlots$maturityPopulation <- pic
  ...
  if(.an(lgtFleets)[1] != 0){
    ...
    inputPlots$lengthComposition1 <- lengthComposition1
    ...
    inputPlots$lengthComposition2 <- lengthComposition2
  }
  ...
  if(Nfleets > 1){
    ...
    fitPlots$totalCatchByFleet <- pic
  }
  ...
  fitPlots$catchResidualsByFleet <- pic
  ...
  fitPlots$absoluteResidualCatchByFleet <- pic
  ...
  if(.an(ageFleets)[1] != 0){
    ...
    fitPlots$residualsCatchAtAgeByFleet <- pic
  }
  ...
  if(.an(lgtFleets)[1] != 0){
    ...
    fitPlots$residualsCatchAtLengthByFleet <- pic
  }
  ...
  if(.an(ageFleets)[1] != 0){
    ...
    fitPlots$ageFitsCatch <- ageFitsCatch
  }
  ...
  if(.an(lgtFleets)[1] != 0){
    ...
    fitPlots$lengthFitsCatch <- lengthFitsCatch
  }
  ...
  fitPlots$predictedObservedCatchesByFleet <- pic
  ...
  fitPlots$predictedObservedIndices <- pic
  ...
  if(.an(ageSurveys)[1] != 0){
    ...
    fitPlots$ageFitsSurvey <- ageFitsSurvey
  }
  ...
  fitPlots$standardizedSurveyResiduals <- pic
  ...
  fitPlots$selectivityFisheryByPentad <- pic  
  ...
  fitPlots$selectivitySurveyByPentad <- pic    
  ...
  if(.an(lgtFleets)[1] != 0){
    ...
    fitPlots$fisheryMeanLength <- pic
  }
  ...
  summaryData <- data.frame(summaryData,stringsAsFactors=F)
  ...
  fitPlots$uncertaintyKeyParams <- pic
  ...
  fitPlots$matureInmatureFishes <- pic
  ...
  fitPlots$fishedUnfishedBiomass <- pic
  ...
  projectionsPlots$ssbPrediction <- pic
  ...
  projectionsPlots$catchPrediction <- pic    
  ...
  yprPlots$yieldSsbPerRecruit <- pic
...
.LikeTable = function(lstOuts){
  if(class(lstOuts)[1]=='jjm.output') {Name=lstOuts$output$info$model;lstOuts=list(lstOuts$output$output)
  } else {Name=lstOuts$info$model;lstOuts=lstOuts$data}
  names(lstOuts)=Name
...
.Fut_SSB_SD = function(lstOuts){
  ...
  #                                    sep="_"),
  #                              sep="_")
  fut$modelscenario <- paste(rep(names(lstOuts),lapply(lstOuts,function(x){nrow(x$SSB_fut_1)*length(grep("SSB_fut_",names(x)))})),
                             paste("Scen",
                                   as.vector(do.call(c,lapply(lstOuts,function(x){rep(1:length(grep("SSB_fut_",names(x))),each=nrow(x$SSB_fut_1))}))),
                                   sep="_"),
                             sep="_")
...
.SSB_SD = function(lstOuts){
  ...
  if(length(lstOuts)>1){ SSB_SD$model=rep(names(lstOuts),lapply(lstOuts,function(x){nrow(x$SSB)}))}
  return(SSB_SD)
}
