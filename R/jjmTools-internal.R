...
.LikeTable = function(lstOuts){
  ...
  names(lstOuts) <- Name
  tab <- cbind(lstOuts[[1]]$Like_Comp_names,
...
.compareModels <- function(lstObject, outputFilename, plotType = "pdf", comparisonType = "time",
  ...
  # Prepare output filename for plots
  outputComparisons <- paste0(outputComparisons, ".", plotType)
  outputComparisons <- file.path(resultPath, outputComparisons)
