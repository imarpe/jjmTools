...
# auxiliary functions
# CompareTime ------------------------------------------------------------
.compareTime <- function(lstOuts,Slot='SSB',SD=F,Sum=NULL,startYear=NULL,legendPos="topright",...){
  
  lst     <- list(...)
  dat     <- lapply(lstOuts,function(x){return(x[[Slot]])})# get selected data
  nms     <- names(dat); if(!is.null(Sum)){nms <- c(nms,paste(Sum[1],"+",Sum[2],sep=""))}
...
.compareMatrix <- function(lstOuts,Slot='TotF',Sum=NULL,YrInd=FALSE,Apply=mean,startYear=NULL,legendPos="topright",...){
...
#   box()
# }
getComparisons = function(lstOuts,comparisonType = "time",Slot=NULL, SD=FALSE, Sum=NULL, startYear=NULL, legendPos="topright",
...
.getPath2 <- function(path, pattern, target)
{
  ...
  return(output)
>>>>>>> ce502759acffc8f23fd8655ed2887d2812c42e2b
}