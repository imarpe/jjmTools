...
Una vez generados los objetos para un modelo simple (jjm.output) y una lista de modelos (lstOuts), se generará un objeto con los gráficos de diagnóstico (solo disponible para modelo simple) de la siguiente manera:

`diagPlots <- diagnostics(outputObject = model)`

A continuación, puede usarse la función `print` para los objetos creados, con lo que se obtendrá:
`print(model)`

````{r, echo = FALSE}
require(jjmTools)
modelName <- "mod4.2"
model <- readJJM(path =  "F:\\NvTomus InDemonic\\ADMB\\JJM\\Code\\admb", modelName)
print(model)
````
...
require(jjmTools)
compareList <- c("mod4.1", "mod4.2", "mod4.3", "mod4.4")
lstModel <- readJJM(path =  "F:\\NvTomus InDemonic\\ADMB\\JJM\\Code\\admb", compareList)
print(lstModel)
````
...
A continuación, puede usarse la función `summary` para los objetos creados, con lo que se obtendrá:
`summary(model)`
...
A continuación, puede usarse la función `plot` para los objetos creados, con lo que se obtendrá:
